...
function getRowsNumberCount() {
  ...
  return api
    .post("/tipoobra/TipoObraCount", req, {
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
      },
...
function onClickEliminar() {
  if (selected.value.length == 0) {
    mostrarMensaje(
      "Eliminar tipos de obra",
      "No se han seleccionado tipos de obra para eliminar",
      "adv"
    );
  } else {
    $q.dialog({
      title: "Eliminar tipos de obra",
      message: "¿Confirma que desea eliminar las tipos de obra seleccionadas?",
      ok: "Aceptar",
      cancel: "Cancelar",
      persistent: true,
    })
...
function Eliminar(indice) {
  const req = {
    idTipoObra: selected.value[indice].idTipoObra,
  };

  api
    .post("/tipoobra/TipoObraDelete", req, {
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
      },
    ...
    .then((response) => {
      ...
      if (resp.codigo == 1) {
        ...
        mostrarMensaje(
          "Eliminar tipos de obra",
          "Error al eliminar el tipo de obra " +
...
function onClickGuardar() {
  if (ValidarFormulario()) {
    if (registro.value.idTipoObra == 0) {
      const req = {
      ...
      api
        .post("/tipoobra/TipoObraInsert", req, {
          headers: {
            "Content-Type": "application/json",
            "Access-Control-Allow-Origin": "*",
          },
        ...
        .then((response) => {
          ...
          if (resp.resultado > 0) {
            registro.value.idTipoObra = resp.resultado;

            guardarLogSistema(
              $q.localStorage.getItem("idusuario"),
              "/Catalogos/TipoObra",
              "Agregar tipo de obra",
              "tipo de obra " + registro.value.nombre + " creado con éxito",
              "Información"
            );
            Editar.value = false;
            onClickBuscar();
            ...
            mostrarMensaje(
              ...
              "Ocurrió un error al agregar el tipo de obra: " + resp.mensaje,
              "err"
            );
          }
        })
        .catch((error) => {
          mostrarMensaje(
            "Guardar tipo de obra",
            "Ocurrió un error al agregar el tipo de obra: " + error,
            "err"
          );
        });
    } else {
      const req = {
        idTipoObra: registro.value.idTipoObra,
        nombre: registro.value.nombre,
        idClaveTrabajo: claveModel.value.idClaveTrabajo,
      };

      api
        .post("/tipoobra/TipoObraUpdate", req, {
          headers: {
            "Content-Type": "application/json",
            "Access-Control-Allow-Origin": "*",
          },
        ...
        .then((response) => {
          ...
          if (resp.resultado > 0) {
            ...
            guardarLogSistema(
              ...
              "tipo de obra " + registro.value.nombre + " modificado con éxito",
              "Información"
            );
            Editar.value = false;
            onClickBuscar();
            ...
            mostrarMensaje(
              "Guardar tipo de obra",
              "Ocurrió un error al modificar el tipo de obra: " + resp.mensaje,
              "err"
            );
          }
        })
        .catch((error) => {
          mostrarMensaje(
            "Guardar tipo de obra",
            "Ocurrió un error al modificar el tipo de obra: " + error,
            "err"
          );
        });
    }
  ...
  <div>
    <q-page class="bg-grey-3 qpage">
      <q-card class="q-pa-md q-gutter-sm" bordered
        ><q-card-section>
          <div class="text-h6">Tipos de Obra</div>
        </q-card-section>
        <q-separator />
        <q-card-section>
          <div class="row">
        ...
        <q-card-section>
          <q-table
            ...
            no-data-label="No hay registros de tipos de obra"
    ...
    <q-dialog v-model="Editar" persistent>
      <q-card style="width: 50%">
        <q-card-section class="bg-grey-4">
          <div class="text-h6">Agregar/Editar tipo de obra</div>
        </q-card-section>
        <q-separator />
