...
function calcularTotalEjercicio(ejercicio: any): number {
  ...
  const pendiente = Number(ejercicio.pendiente) || 0;
...
function calcularEjercicio(ejercicio: any) {
  ...
  ejercicio.total = subtotal + importePendiente;
}
...
function eliminarEjercicio(index: number) {
  if (ejercicios.value.length === 1) {
    $q.notify({
      type: "warning",
      message: "Debe existir al menos un ejercicio.",
    });
...
function cargarValoresIniciales(ejercicio: any) {
  if (opcionesLinea.value.length > 0) {
    ejercicio.linea = opcionesLinea.value[0];

    cargarTiposObra(ejercicio);
  }

  if (opcionesMaterial.value.length > 0) {
    ejercicio.material = opcionesMaterial.value[0];
  }

  if (opcionesExcavacion.value.length > 0) {
    ejercicio.excavacion = opcionesExcavacion.value[0];
  }
}

async function cargarTiposObra(ejercicio: any) {
  const req = {
    idTipoObra: 0,

    idClaveTrabajo: ejercicio.linea.idClaveTrabajo,
  ...
  try {
    const response = await api.post("/tipoobra/TipoObraSelect", req);

    const resp = response.data;

    if (resp.codigo === 1) {
      ejercicio.opcionesObra = resp.tiposObra || [];

      ejercicio.obra = ejercicio.opcionesObra[0] || {
        idTipoObra: 0,
        idClaveTrabajo: 0,
        nombre: "",
      };
...
async function cargarTuberias(ejercicio: any) {
  const req = {
    idRelacion: 0,

    idLineaTrabajo: ejercicio.linea.idLineaTrabajo,

    idTipoObra: ejercicio.obra.idTipoObra,
  ...
  try {
    ...
    if (resp.codigo === 1) {
      ejercicio.opcionesTuberia = resp.relacionesLineaObraTuberia || [];

      ejercicio.tuberia = ejercicio.opcionesTuberia[0] || {
        idRelacion: 0,
        idLineaTrabajo: 0,
        idTipoObra: 0,
        idTuberia: 0,
...
async function cargarDiametros(ejercicio: any) {
  const req = {
    idRelacion: 0,

    idRelacionObra: ejercicio.tuberia.idRelacion,
  ...
  try {
    ...
    if (resp.codigo === 1) {
      ejercicio.opcionesDiametro = resp.relacionesObraDiametro || [];

      ejercicio.diametro = ejercicio.opcionesDiametro[0] || {
        idRelacion: 0,
        idRelacionObra: 0,
        idDiametro: 0,
        diametro: "",
...
async function encontrarPrecioFinal(ejercicio: any) {
  const req = {
    idRelacion: 0,

    idRelacionDiametro: ejercicio.diametro.idRelacion,

    idTipoMaterial: ejercicio.material.idTipoMaterial,

    idExcavacion: ejercicio.excavacion.idExcavacion,
  ...
  try {
    ...
    if (resp.codigo === 1) {
      if (resp.relacionesFinalPrecio && resp.relacionesFinalPrecio.length > 0) {
        ejercicio.precio = resp.relacionesFinalPrecio[0];

        ejercicio.calcularHabilitado = true;
      } else {
        ejercicio.precio = {
          idRelacion: 0,
        ...
        ejercicio.calcularHabilitado = false;
      }
    ...
    ejercicio.cargandoPrecio = false;
  }
}
...
function configurarWatchers(ejercicio: any) {
  ...
  watch([() => ejercicio.metros, () => ejercicio.pendiente], () => {
    calcularEjercicio(ejercicio);
  });
}
...
async function cargarCombosInicio() {
  ...
  try {
    ...
    const responseLinea = await api.post(
      "/lineatrabajo/LineaTrabajoSelect",
      reqLinea
    );

    const respLinea = responseLinea.data;

    if (respLinea.codigo === 1) {
      opcionesLinea.value = respLinea.lineasTrabajo || [];
    }
    ...
    const responseMaterial = await api.post(
      "/tipomaterial/TipoMaterialSelect",
      reqMaterial
    );

    const respMaterial = responseMaterial.data;

    if (respMaterial.codigo === 1) {
      opcionesMaterial.value = respMaterial.tiposMaterial || [];
    }
    ...
    const responseExcavacion = await api.post(
      "/excavacion/ExcavacionSelect",
      reqExcavacion
    );

    const respExcavacion = responseExcavacion.data;

    if (respExcavacion.codigo === 1) {
      opcionesExcavacion.value = respExcavacion.excavaciones || [];
    }

    cargarValoresIniciales(ejercicios.value[0]);
  } catch (error) {
    console.error(error);
  } finally {
    loading.value = false;
    ...
    <q-card bordered class="q-pa-md">
      <q-card-section>
        <div class="text-center">
          <img src="../assets/logo-sapal.png" style="max-width: 180px" />
        ...
        >
          <template #header>
            <q-item-section avatar>
              <q-avatar color="primary" text-color="white" icon="calculate" />
            </q-item-section>
          ...
          <q-card-section>
            ...
            <div class="row">
              ...
              <div class="q-pa-sm col-12 col-md-6">
                <q-input
                  v-model.number="ejercicio.metros"
                  outlined
                  type="number"
                  label="Metros"
                  min="0"
