...
function seleccionarRegistro(e, obj, index) {
  window.location.href = "#/Configuracion/GrupoDetalle?IDGrupo=" + obj.idGrupo;
}

function onClickNuevo() {
  window.location.href = "#/Configuracion/GrupoDetalle?IDGrupo=0";
}
...
function onClickEliminar() {
  if (selected.value.length == 0) {
    mostrarMensaje(
      "Eliminar grupos",
      "No se ha seleccionado grupos para eliminar",
      "adv"
    );
  } else {
    $q.dialog({
      title: "Eliminar grupos",
      message: "¿Confirma que desea eliminar los grupos seleccionados?",
      ok: "Aceptar",
      cancel: "Cancelar",
      persistent: true,
    })
...
function Eliminar(indice) {
  const req = {
    idGrupo: selected.value[indice].idGrupo,
  };

  api
    .post("/grupo/GrupoDelete", req, {
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
      },
    ...
    .then((response) => {
      ...
      if (resp.codigo == 1) {
        guardarLogSistema(
          ...
          "/Configuracion/GrupoLista",
          "Eliminar grupo",
          "Grupo " + selected.value[indice].nombre + " eliminado con éxito",
          "Información"
        );
    ...
    .catch((error) => {
      ...
      mostrarMensaje(
        "Eliminar grupos",
        "Error al eliminar grupo " + selected.value[0].nombre + ": " + error,
        "err"
      );
    });
}
...
<template>
  <q-page class="bg-grey-3 qpage">
    <q-card class="q-pa-md q-gutter-sm" bordered
      ><q-card-section>
        <div class="text-h6">Grupos de seguridad</div>
      </q-card-section>
      <q-separator />
      <q-card-section>
        <div class="row">
      ...
      <q-card-section>
        <q-table
          ...
          no-data-label="No hay registros de grupos"
